...
Z1$sim()
>>>>>>> 730efabc11f004a4de3d0b34b79fafed4335af1d
...
One issue we then faced was to merge our datasets all together into a two-dimension table whilst we had three dimensions to take into account: country, year and variable. Therefore, for each dataset, we moved the years from columns to rows, without getting rid of the country labels of course. Thus the first column would be the list of countries, the second the list of years (same for each country: 2001 ... 2013), the third the variable of interest.  

We merged the datasets two by two until we we obtained the final dataset that would allow us to test the data and have an attempt at one or two regressions. 

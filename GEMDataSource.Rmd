...
PerceivedOpportunities <- repmis::source_data("https://raw.githubusercontent.com/SamDund/Assign2Education/master/GEMData/GEM_PerceivedOpportunities.csv", header = TRUE, cache = FALSE)
...
R1 <- lm(PercentIntention ~ PercentPerceived + PercentFear, data = FinalGEM)
summary(R1)
...
# Run a linear regression
R2 <- lm(PercentNascent ~ PercentPerceived + PercentFear, data = FinalGEM)
summary(R2)
...
# Run a linear regression
R3 <- lm(PercentOwner ~ PercentPerceived + PercentFear, data = FinalGEM)
summary(R3)
...
# Plot graph
car::scatterplotMatrix(FinalGEM)
...
CompeteDataset <- merge(FinalGEM, GDPGrowth,
                   union("Country", "Year"),
                   all = TRUE) 

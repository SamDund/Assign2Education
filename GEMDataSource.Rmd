...
In order to scrape our data, we used repmis, tidyr, reshape2, plyr, WDI, stargazer and Zelig packages before importing one dataset per variable mentioned above:  
...
We cleaned our data by getting rid of unnecessary characters like the "X" to which we substituted ""nothing. We called the new versions with different names.

One issue we then faced was to merge our datasets all together into a two-dimension table whilst we had three dimensions to take into account: country, year and variable. Therefore, for each dataset, we moved the years from columns to rows, without getting rid of the country labels of course. Thus the first column remained be the list of countries, the second became the iterative list of years (same list for each country: 2001 ... 2013), the third was the variable for the variable of interest.  

We merged the datasets two by two until we obtained the final GEM data set.

To merge that final GEM dataset with the annual GDP Growth dataset, we had to recode some country labels that differed (USA, South Korea) and to remove the iso2c data.

We replaced the few missing values by NA.  


#### Data Description


For all 33 OECD countries we should have 13 observations, one per year from 2001 to 2013, for each variable. Of course, there are a few missing values, especially regarding the perceived opportunities for some countries, namely Estonia, Luxembourg and Slovak Republic.

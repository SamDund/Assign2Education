...
CompleteDataset[,"AnnualGDPGrowth"] <- as.numeric(CompleteDataset[,"AnnualGDPGrowth"])
...
VectorData <- as.vector(CompleteDataset)

class(CompleteDataset$GDPGrowth)

z.out <- zelig(as.factor(PercentIntention) ~ PercentPerceived + PercentFear*PercentOpportunities + AnnualGDPGrowth, cite = FALSE, data = VectorData, model = "oprobit")


x.out1 <- setx(z.out, PercentPerceived=10)

x.out2 <- setx(z.out, PercentPerceived=100)
...
Regarding entrepreneurship, we intend to look into three dimensions. We intend to look at established entrepreneurship in the country, whether it is successful or not, nascent entrepreneurship (less than three months), again whether it is successful or not, and entrepreneurial intention. We will study them successively as we may well observe different effects.
=======
...
We look at the average "perceived capabilities" of an individual (18-64) of a given country to start a business.

We also take into account how likely the "fear of failure" could deter the average individual (18-64) of a country to become an entrepreneur.  
...
Besides these nine conditions and the GEM "perceived opportunities" measure, we also intend to control for the economic outlook at the time of data collection which is why we chose to include growth rate for each country from 2001 until 2013. This is our only data that does not originates from the GEM collection.

>>>>>>> 669f5e9ecb2c19a4150a5416abf37d616e8a76b4

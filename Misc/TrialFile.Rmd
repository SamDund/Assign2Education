...
LongGEM<- melt(data = CleanMerge,
                  id.vars = c("Country"),
                  measure.vars = c("2001", "2002", "2003", "2004", "2005", "2006",
                                   "2007", "2008", "2009", "2010"))

# Rename columns
names(CleanMerge) <- gsub("variable", "Year", names(CleanMerge))
names(CleanMerge) <- gsub("value", "Support", names(CleanMerge))
...
# Subset LongGEM with the countries we need  (Why does 2001 and 2010 data disappear?)
subset_data <- subset(CleanMerge, Country %in% myvars)

# Delete unnecessary column
subset_data$row.names <- NULL

# Arrange subset_data by country and year
GData <- arrange(subset_data, Country, Year) 

GData <- GData[order("Country",)]

...
Even when focused on attitudes / Entrepreneurship,  
Studies look more at what attitudes are favourable  
Almost none searched what determines these attitudes.


## Causal Chain

Education System that reinforces Self-confidence  

Higher degree of individual self-esteem  

Higher probability an individual becomes an entrepreneur  
...
- GEM is being used is regularly refered to in research, including in OECD reports. 

- GEM uses primary data collection which results in massive comprehensive datasets. That is the 2001-2010 multi-year APS Database covers over 1.3 million observations over 85 countries.  

- APS or Adult Population Survey (at least 2000 sample size) looks at activity, attitudes and aspirations.  
...
Variable | Indicator 
-------------|---------------
Y Dependent Vaiable | Rate of Entrepreneurship (Intention, Nascent, Established)
...
## Explaining Way of Teaching

In the absence of indicator directly aligned with our research design, we turned to an item of the NES survey as a proxy, D01:
...
C2 <- countrycode(PerceivedCapabilities$Country, "country.name", "iso2c")

# Create a new column and insert the country code vector
PerceivedCapabilities$new.col <- C2

# Reorder columns so country code is first
C3 <- PerceivedCapabilities[,c(15,1:14)]
...
names(C4) <- gsub("X", "", names(C4))                    
  
# Set columns as numeric
C4[,"2001"] <- as.numeric(C4[,"2001"])
C4[,"2002"] <- as.numeric(C4[,"2002"])
C4[,"2003"] <- as.numeric(C4[,"2003"])
C4[,"2004"] <- as.numeric(C4[,"2004"])
C4[,"2005"] <- as.numeric(C4[,"2005"])
C4[,"2006"] <- as.numeric(C4[,"2006"])
C4[,"2007"] <- as.numeric(C4[,"2007"])
C4[,"2008"] <- as.numeric(C4[,"2008"])
C4[,"2009"] <- as.numeric(C4[,"2009"])
C4[,"2010"] <- as.numeric(C4[,"2010"])
C4[,"2011"] <- as.numeric(C4[,"2011"])
C4[,"2012"] <- as.numeric(C4[,"2012"])
C4[,"2013"] <- as.numeric(C4[,"2013"])

# Save cleaned df
Cap1 <- C4

# Generate a new column with mean of all other columns
C4$mean <- rowMeans(subset(C4, select = c("2001", "2002", "2003", "2004", "2005", "2006", "2007", "2008",
...
# Create 
C4[, 3] <- round(log(C4[, 3]), digits = 2)
```

```{r echo = FALSE, cache= TRUE, include= TRUE, results= 'asis'}

# Plot perceived opportunties map
Cap1_map <- gvisGeoChart(C4, locationvar = 'iso2c',
                      colorvar = 'mean',
                      options = list(
                        colors = "['#ece7f2', '#2b8cbe']"
))

# Duplicate D4 (perceived capabilities) df
Cap4 <- C4
...
# Subset Cap4
Cap5 <- subset(Cap4, iso2c %in% myvars2)
...
# Edit column names
names(Cap6) <- gsub("value", "Capabilities", names(Cap6))
names(Cap6) <- gsub("variable", "Year", names(Cap6))
...
# Create plot
ggplot(Cap6,aes(x = Year, y = PerceivedCapabilities, colour = Country, group = Country)) + geom_line()
